...
describe("HoldingSparkline", () => {
  // Every render needs the i18n provider — the sparkline now uses
  // `useTranslations("Holdings")` for its row label and aria-label
  // strings (#478 review #3).
  it("renders one polyline point per value", () => {
    const { container } = wrap(<HoldingSparkline values={[1, 2, 3, 4]} />);
    expect(points(container)).toHaveLength(4);
  });

  it("renders nothing for fewer than two points", () => {
    const { container } = wrap(<HoldingSparkline values={[5]} />);
    expect(container.querySelector("svg")).toBeNull();
  });

  it("colors an upward series green and a downward series red (by first→last)", () => {
    const up = wrap(<HoldingSparkline values={[1, 5]} />);
    expect(up.container.querySelector("svg")?.getAttribute("class")).toContain("text-success");
    const down = wrap(<HoldingSparkline values={[5, 1]} />);
  ...
  it("draws a flat midline (no NaN) when all values are equal", () => {
    const { container } = wrap(<HoldingSparkline values={[3, 3, 3]} />);
  ...
  it("is keyboard-focusable with a translated aria-label summarizing the range", () => {
    // Needs the i18n provider so `Holdings.sparklineRangeAria` resolves.
    wrap(<HoldingSparkline values={[1, 5, 2, 8, 3]} />);
    const svg = document.querySelector("svg");
    expect(svg).not.toBeNull();
    // #478 review #6: drop role="img" — the SVG is interactive (focusable,
    // hoverable, with a tooltip), so a bare aria-label is the right pattern.
    ...
    // The translated "Price range" prefix (Holdings.sparklineRangeAria) is
    // present in any locale; the test runtime is en by default.
    expect(label).toMatch(/price range/i);
  ...
  it("shows a floating tooltip on mouseenter with the formatted range", () => {
    ...
    // The row label is translated via Holdings.sparklineRange → "Range".
